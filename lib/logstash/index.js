...
function buildMessage(getType, codec, level, module, message, data) {
    return new Promise(resolve => {
        ...
        resolve(
            _.merge(
                {},
                mapConflictingDataProperties(data, baseObject),
                baseObject
            )
        );
    });
...
module.exports = function LogstashLogger(config) {
    const Transport = require(`./senders/${config.output.transport}`);
    const transport = new Transport(config.output);
    const getType = getLogstashType.bind(undefined, config.eventType);
    const buildMessageWithGetType = buildMessage.bind(
        undefined,
        getType,
        config.codec
    );
